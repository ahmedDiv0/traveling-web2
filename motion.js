const labels = document.querySelectorAll('.form-cont label')
labels.forEach(label => {
    label.innerHTML = label.innerText
    .split('')
    .map((letter , idx) => `<span
     style="transition-delay:${idx * 100}ms">${letter}</span>`)
    .join('')
})

//panal in the frist page
...
function setActiveSlide(){
    slides.forEach ( (slide) => slide.classList.remove('activer'))

    slides[activeSlide].classList.add('activer')
}


//toggle the box under the header
...
const boxes =document.querySelectorAll('.scrol')
...
function checkBoxes(){
    ...
    boxes.forEach(scrol => {
        const boxTop = scrol.getBoundingClientRect().top
...
const img = document.querySelectorAll('.scrol2')
window.addEventListener("scroll" ,checkImge)
checkImge()
function checkImge(){
    const tiggerBottom = window.innerHeight / 5 * 4
    img.forEach(scrol2 =>{
        const imgTop = scrol2.getBoundingClientRect().top
        if(imgTop<tiggerBottom){
            scrol2.classList.add('show2')
        }else{
            scrol2.classList.remove('show2')
        }
    })
}
